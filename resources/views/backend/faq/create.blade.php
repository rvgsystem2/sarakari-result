<x-app-layout>
    ...
    <div class="max-w-xl mx-auto p-6">
        <form action="{{ route('faq.store') }}" method="POST" class="space-y-4 bg-white shadow-lg rounded-lg p-6">
            @csrf

            <div>
                <label class="block font-medium">Question</label>
                <input type="text" name="question" class="w-full border rounded-lg p-2" required>
            </div>

            <div>
                <label class="block font-medium">Answer</label>
                <textarea name="answer" rows="4" class="w-full border rounded-lg p-2" required></textarea>
            </div>

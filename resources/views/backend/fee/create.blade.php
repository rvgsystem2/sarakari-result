...
<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold">Add New Fee</h2>
    </x-slot>

    <div class="max-w-lg mx-auto py-6">
        <form action="{{ route('fee.store') }}" method="POST" class="space-y-4 bg-white p-6 rounded-lg shadow">
            @csrf

            <div>
                <label class="block font-medium">Group</label>
                <input type="text" name="group" value="{{ old('group') }}" class="w-full border rounded-lg p-2">
                @error('group') <p class="text-red-600 text-sm">{{ $message }}</p> @enderror
            </div>

            <div>
                <label class="block font-medium">Category</label>
                <input type="text" name="category" value="{{ old('category') }}" class="w-full border rounded-lg p-2">
                @error('category') <p class="text-red-600 text-sm">{{ $message }}</p> @enderror
            </div>

            <div>
                <label class="block font-medium">Amount</label>
                <input type="number" step="0.01" name="amount" value="{{ old('amount') }}" class="w-full border rounded-lg p-2">
            ...
            <div>
                ...
                <input type="text" name="payment_modes" value="{{ old('payment_modes') }}" class="w-full border rounded-lg p-2">
